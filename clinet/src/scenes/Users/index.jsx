...
const Users = () => {
  ...
  const columns = [
    ...
    { field: "updatedAt", headerName: "Updated At", flex: 1, editable: false },
    {
      ...
      renderCell: (params) => (
        <>
          <Button variant="contained" color="primary" startIcon={<EditIcon sx={{ marginLeft: "8px" }} />} onClick={() => openDialog(params.row)} sx={{ width: 25, margin: "4px 4px" }}></Button>
          <Button variant="contained" color="error" startIcon={<DeleteIcon sx={{ marginLeft: "8px" }} />} onClick={() => handleDeleteUser(params.row.id)} sx={{ width: 50, margin: "4px 4px" }}></Button>
        </>
      ),
    },
  ];
  ...
  return (
    <Box m="2.5rem">
      <Header title="Admins & Super Admins" subtitle="Manage Admins and Super Admins" />
      <Typography variant="body1" gutterBottom sx={{ mt: 2, mb: 4 }}>
        <Box display="flex" alignItems="center">
          <CheckIcon sx={{ mr: 1 }} />
          <Box>This page serves as the backoffice for creating and managing admin accounts for the platform. You can add, edit, or delete admin users using the functionalities provided below.</Box>
        </Box>
      </Typography>
          ...
          <Dialog open={isDialogOpen} onClose={closeDialog}>
            ...
            <DialogContent>
              {["firstName", "lastName", "email"].map((field) => (
                <TextField key={field} autoFocus={field === "firstName"} margin="dense" id={field} label={field.charAt(0).toUpperCase() + field.slice(1)} type={field === "email" ? "email" : "text"} fullWidth value={newUserData[field]} onChange={handleInputChange} />
              ))}
              {!selectedUser && ( // Render password field only if adding new user
                <TextField autoFocus={false} margin="dense" id="password" label="Password" type="password" fullWidth value={newUserData.password} onChange={handleInputChange} />
              )}
              <Box mt={2}>
